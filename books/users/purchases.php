...
<aside id="sb" role="navigation">
  ...
  <div class="sb-sect">Compte</div>
    <a class="sb-item" href="../dashboard.php">
      <i class="bi bi-grid-1x2"></i> Dashboard
    </a>
  <nav class="sb-nav">
    <div class="sb-sect">Ma bibliothèque</div>
    <a class="sb-a" href="../books/index.php"><i class="bi bi-compass"></i> Explorer</a>

    <div class="sb-sect">Personnel</div>
          ...
          <div class="pur-card" data-id="<?= (int)$p['achat_id'] ?>" style="animation-delay:<?= $i*.04 ?>s">
            ...
            <div class="pur-body">
              ...
              <div class="pur-actions">
                ...
                <!-- Télécharger -->
                <?php if ($pdfPath): ?>
                <button class="ic-btn" onclick="downloadPdf(<?= (int)$p['livre_id'] ?>, <?= json_encode($pdfPath,JSON_HEX_TAG) ?>)" title="Télécharger PDF" type="button">
                  <i class="bi bi-cloud-download"></i>
                </button>
                <?php endif; ?>

                <!-- Favori -->
                <button class="ic-btn <?= $isFav?'fav':'' ?> fav-btn"
                        data-livre-id="<?= (int)$p['livre_id'] ?>"
                        onclick="toggleFav(this, <?= (int)$p['livre_id'] ?>)"
                        title="<?= $isFav?'Retirer des favoris':'Ajouter aux favoris' ?>" type="button">
                  <i class="bi bi-<?= $isFav?'star-fill':'star' ?>"></i>
                </button>

                <!-- Note -->
                <button class="ic-btn" onclick="openReview(<?= (int)$p['livre_id'] ?>, <?= json_encode($p['titre']??'',JSON_HEX_TAG) ?>, <?= $myNote ?>)" title="Évaluer" type="button">
                  <i class="bi bi-chat-square-quote"></i>
                </button>

                <!-- Facture -->
                <button class="ic-btn" onclick="openInvoice(<?= (int)$p['achat_id'] ?>)" title="Voir la facture" type="button">
